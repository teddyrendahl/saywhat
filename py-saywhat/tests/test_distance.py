...
# The following tests cases were taken from the nltk library
...
def test_with_transpositions(
        left: str, right: str, substitution_cost: int, expecteds: Tuple[int, int]
):
    """
    ...
    for s1, s2 in ((left, right), (right, left)):
        ...
        for expected, transpositions in zip(expecteds, [True, False]):
            predicted = saywhat.edit_distance(
                s1,
                s2,
